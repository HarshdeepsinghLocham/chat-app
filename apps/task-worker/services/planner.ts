...
type CreateOrRefreshTaskPlanOptions = { llmRequestFn?: PlannerLlmRequestFn; persist?: boolean };
...
function buildFallbackPlan(context: PlannerContext): { goal: string; successDefinition: string; steps: ITaskStep[] } {
    const toolCandidates = context.availableTools.slice(0, 3).map((tool: { name: string; description: string }) => ({
        toolName: tool.name,
        confidence: 0.5,
...
function buildGeneratedTaskPlan(
    context: PlannerContext,
    plan: { goal: string; successDefinition: string; steps: ITaskStep[] },
    planNotes: string
): GeneratedTaskPlan {
...
export async function createOrRefreshTaskPlan(context: PlannerContext, options?: CreateOrRefreshTaskPlanOptions) {
    ...
    const generatedPlan = buildGeneratedTaskPlan(
        context,
        plan,
        llmPlan ? "Generated by LLM planner." : "Fallback deterministic plan."
    );
    ...
    return TaskPlanModel.findOneAndUpdate(
        { taskId: context.taskId },
        {
            $set: {
                ...
                steps: plan.steps,
                activeStepId: plan.steps[0]?.stepId ?? null,
                planNotes: generatedPlan.planNotes,
            },
        },

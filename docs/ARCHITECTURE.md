...
| **Eng rule** | With `EXECUTION_MODE_ENFORCE=1` (code default), false auto-execute under effective `suggest_only` is a **P0** product bug ([ADR-005](./decisions/ADR-005-suggest-first-work-coordination.md) Consequences). `EXECUTION_MODE_ENFORCE=0` is deprecated shadow-only (still honored this release; logs would-gate; legacy auto-execute may continue). |
